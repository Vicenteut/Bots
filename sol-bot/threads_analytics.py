...
def fetch_followers() -> dict[str, Any]:
    """Fetch follower count via Threads user insights API."""
    token = _load_token()
    if not token:
        return {"count": None, "error": "no token"}
    params = urllib.parse.urlencode({"metric": "followers_count", "access_token": token})
    try:
        data = _get(f"{BASE_URL}/me/threads_insights?{params}")
        for item in data.get("data", []):
            if item.get("name") == "followers_count":
                tv = item.get("total_value") or {}
                if "value" in tv:
                    return {"count": int(tv.get("value") or 0), "error": None}
                vals = item.get("values") or []
                if vals:
                    return {"count": int(vals[0].get("value") or 0), "error": None}
    except Exception as exc:
        return {"count": None, "error": _safe_error(exc)}
    return {"count": None, "error": "metric not returned"}
...
def init_db(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_follower_snapshots_at ON follower_snapshots(snapshot_at);
    ...
    # v2: source attribution (idempotent ALTER + index)
    try:
        conn.execute("ALTER TABLE posts ADD COLUMN source_name TEXT")
...
def set_post_source(post_id: str, source_name: str | None) -> None:
    """Persist the upstream source channel for a freshly-published Threads post.
    Idempotent: only writes if the existing source_name is NULL/empty so that
    sync_posts metadata refreshes don't clobber a previously-set source.
    """
    ...
    try:
        with _connect() as conn:
            conn.execute(
                "UPDATE posts SET source_name = ? "
                "WHERE post_id = ? AND (source_name IS NULL OR source_name = \'\')",
                (source_name, str(post_id)),
            )
            conn.commit()
    except Exception:
        # Never let analytics bookkeeping break a publish flow.
        pass
...
def get_analytics(
    ...
    with _connect(db_path) as conn:
        ...
        # ─── EXTENSIONS v2: summary_prev + time_series + heatmap + followers ───
        # Previous period summary (same length window, immediately before `since`)
        since_prev = (datetime.now(timezone.utc) - timedelta(days=days * 2)).isoformat()
        summary_prev_row = conn.execute(
            cte + """
            SELECT
                COUNT(*) AS posts,
                COALESCE(SUM(latest.views), 0) AS views,
        ...
        # Daily time series for chart (impressions, engagements, posts, avg_eng_rate)
        time_series = [
            _row_to_dict(row) for row in conn.execute(
                cte + """
                SELECT
                    strftime('%Y-%m-%d', COALESCE(REPLACE(p.timestamp, '+0000', '+00:00'), p.first_seen_at)) AS day,
                    COUNT(*) AS posts,
                    COALESCE(SUM(latest.views), 0) AS impressions,
                    COALESCE(SUM(latest.likes + latest.replies + latest.reposts + latest.quotes), 0) AS engagements,
        ...
        # Heatmap: dow x hour aggregation (SQLite %w: 0=Sun..6=Sat)
        heatmap = [
            _row_to_dict(row) for row in conn.execute(
                cte + """
                SELECT
                    CAST(strftime('%w', REPLACE(p.timestamp, '+0000', '+00:00')) AS INTEGER) AS dow,
                    CAST(strftime('%H', REPLACE(p.timestamp, '+0000', '+00:00')) AS INTEGER) AS hour,
                    COUNT(*) AS posts,
                    ROUND(COALESCE(AVG(latest.engagement_rate), 0), 4) AS avg_eng_rate,
                    ROUND(COALESCE(AVG(latest.views), 0), 0) AS avg_views
                FROM posts p
                JOIN latest ON latest.post_id = p.post_id
                WHERE COALESCE(p.timestamp, p.first_seen_at) >= ?
        ...
        # ─── v2: by_source aggregation (real channel attribution) ───
        try:
            by_source = [
                _row_to_dict(row) for row in conn.execute(
                    cte + """
                    SELECT
                        COALESCE(NULLIF(p.source_name, ''), 'Unknown') AS source,
                        COUNT(*) AS posts,
                        COALESCE(SUM(latest.views), 0) AS views,
